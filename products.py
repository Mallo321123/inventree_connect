...
def update_products_shopware():
    ...
    def request(page, limit):
        try:
            ...
            logging.debug("Shopware Produkte erfolgreich abgerufen")
        ...
        for product in products:
            ...
            if cursor.fetchone() is None:
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (product["id"], product["name"], product["description"], True, product["active"]))
                count_update += 1
                ...
                cursor.execute("""
                    ...
                    WHERE shopware_id = ?
                """, (product["name"], product["description"], product["active"], product["id"]))
                count_update += 1
...
# updates product db from inventree
def update_products_inventree():
    pass
...
def sync_inventree():
    ...
    for product in products:
        ...
        if product[1]:
            product_desc = product[1].replace('<', '').replace('>', '').replace('/', '').replace('\\', '').replace('div', '').replace('span="de"', '')
        else:
            product_desc = ''
